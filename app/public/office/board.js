...
export function mount(el, ctx) {
  ...
  async function load() {
    ...
    if (r.ok) {
      data = r.data
      error = ''
    } else if (r.status !== 401) error = errorText(r)
    paint()
  }
  ...
  function paint() {
    ...
    q('#board-counts').innerHTML = counts
      .map(([label, n]) => `<li data-count="${label}"><span class="count-n">${n}</span>${label}</li>`)
      .join('')
