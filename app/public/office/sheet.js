...
export async function openVisitSheet(visit, onChanged) {
  ...
  const html = () => {
    ...
    const workerOptions = [
      ...
      inactive,
      ...workers.map((w) => `<option value="${w.id}"${w.id === v.worker_id ? ' selected' : ''}>${esc(w.name)}</option>`),
    ].join('')
  ...
  const draw = () => {
    ...
    q('#vs-form').addEventListener('submit', async (e) => {
      ...
      const r = await office('PUT', `/api/office/visits/${v.id}`, {
        worker_id: worker ? Number(worker) : null,
        date: q('#vs-date').value,
        start: q('#vs-start').value,
        end: q('#vs-end').value,
        version: v.version,
      })
      if (await done(r)) {
    ...
    q('#vs-times').addEventListener('submit', async (e) => {
      ...
      for (const [ms, date, hm, field] of [
        [checkIn, inDate, inHm, 'check_in_at'],
        [checkOut, outDate, outHm, 'check_out_at'],
      ]) {
      ...
      const r = await office('PUT', `/api/office/visits/${v.id}/times`, {
        check_in_at: isoOf(checkIn),
        check_out_at: isoOf(checkOut),
        reason: q('#vs-fix-reason').value,
        version: v.version,
      })
      if (await done(r)) {
        timesNote = 'Times saved.'
        draw()
        onChanged(v)
      }
    })
    ...
    for (const el of [inBox, outBox, q('#vs-fix-in'), q('#vs-fix-out')])
      el.addEventListener(el.type === 'checkbox' ? 'change' : 'input', syncOvernight)
    syncOvernight()
