...
export function mount(el, ctx) {
  ...
  function conflictsHtml() {
    const list = data.conflicts
      .map(
        (c, i) => `<li><button type="button" class="conflict" data-conflict-index="${i}" data-kind="${esc(c.kind)}"
        ...
        <span class="conflict-msg">${esc(c.message)}</span></button></li>`,
      )
      .join('')
  ...
  function gridHtml() {
    ...
    const body = rows()
      .map(
        (w) =>
          `<div class="grid-worker">${esc(w.name)}<span class="muted">${esc(w.hours_label)}</span></div>${data.days
            .map(
              (d) =>
                `<div class="cell" data-worker-id="${w.id ?? 'none'}" data-date="${d.date}">${data.visits
                  .filter((v) => v.date === d.date && (v.worker_id ?? null) === w.id)
                  .map((v) => chipHtml(v, sev.get(v.id), on.has(v.id)))
                  .join('')}</div>`,
            )
            .join('')}`,
      )
  ...
  function phoneHtml() {
    const sev = severities()
    const on = lit()
    const tabs = `<div class="day-tabs" role="tablist" aria-label="Day">${data.days
      .map(
        (d) => `<button type="button" role="tab" class="day-tab"
        data-day="${d.date}" aria-selected="${d.date === day}">${esc(d.date_label)}</button>`,
      )
      .join('')}</div>`
    const workerOptions = (v) => {
      const sel = v.worker_id ?? null
      const inactive =
        sel != null && !data.workers.some((w) => w.id === sel)
          ? `<option value="${sel}" selected>${esc(`${v.worker_name} (inactive)`)}</option>`
          : ''
      return [
        `<option value=""${sel == null ? ' selected' : ''}>No worker</option>`,
        inactive,
        ...data.workers.map((w) => `<option value="${w.id}"${w.id === sel ? ' selected' : ''}>${esc(w.name)}</option>`),
      ].join('')
    }
    const groups = rows()
      .map((w) => {
        ...
        if (!vs.length) return ''
        return `<section class="wgroup" data-worker-id="${w.id ?? 'none'}"><h3>${esc(w.name)} <span class="muted">${esc(w.hours_label)}</span></h3>${vs
          .map(
            (
              v,
            ) => `<article class="vcard" data-visit-id="${v.id}" data-conflict="${sev.get(v.id) ?? ''}"${on.has(v.id) ? ' data-highlight' : ''}>
  ...
  function render() {
    ...
    el.innerHTML = data
      ? `${head}${conflictsHtml()}${wide.matches ? gridHtml() : phoneHtml()}`
      : `${head}<p class="empty">Loading the week…</p>`
  ...
  const clearOver = () =>
    el.querySelectorAll('.cell.is-over').forEach((c) => {
