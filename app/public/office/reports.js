...
function payrollHtml(d) {
  ...
  const body = d.rows
    .map(
      (w) => `<tr class="row-worker" data-worker-id="${w.worker_id}"><th scope="row">${esc(w.worker_name)}</th>
      <td class="num">${w.visits}</td><td class="num hours">${esc(w.hours)}</td><td class="num">${esc(w.hm_label)}</td></tr>${w.clients
        .map(
          (
            c,
          ) => `<tr class="row-sub" data-worker-id="${w.worker_id}" data-client-id="${c.client_id}"><td class="indent">${esc(c.client_name)}</td>
      <td class="num">${c.visits}</td><td class="num hours">${esc(c.hours)}</td><td class="num">${esc(c.hm_label)}</td></tr>`,
        )
        .join('')}`,
    )
    .join('')
  ...
  return `<p class="muted report-note">${esc(d.note)}</p>
    ${
      d.rows.length
        ? table('payroll-table', [['Worker and client'], ['Visits', 'num'], ['Hours', 'num'], ['Hours and minutes', 'num']], body, foot)
...
function billingHtml(d) {
  const body = d.funders
    .map(
      (
        f,
      ) => `<tr class="row-funder" data-funder-id="${f.funder_id}"><th scope="row">${esc(f.funder_name)}</th><td class="num">${f.visits}</td>
      <td class="num scheduled">${esc(f.scheduled_hours)}</td><td class="num hours">${esc(f.hours)}</td><td class="num">${esc(f.hm_label)}</td></tr>${f.clients
        .map(
          (
            c,
          ) => `<tr class="row-sub" data-funder-id="${f.funder_id}" data-client-id="${c.client_id}"><td class="indent">${esc(c.client_name)}</td><td class="num">${c.visits}</td>
        <td class="num scheduled">${esc(c.scheduled_hours)}</td><td class="num hours">${esc(c.hours)}</td><td class="num">${esc(c.hm_label)}</td></tr>`,
        )
        .join('')}`,
    )
    .join('')
  ...
  return `<p class="muted report-note">${esc(d.note)}</p>${
    d.funders.length
      ? table(
          'billing-table',
          [['Funder and client'], ['Visits', 'num'], ['Scheduled hours', 'num'], ['Hours worked', 'num'], ['Hours and minutes', 'num']],
          body,
          foot,
        )
      : '<p class="empty">No finished visits in this period.</p>'
...
function missedHtml(d) {
  const body = d.rows
    .map(
      (r) => `<tr data-visit-id="${r.visit_id}" data-what="${esc(r.what)}"><td>${esc(r.date_label)}</td><td>${esc(r.time_label)}</td>
    <td>${esc(r.client_name)}</td><td>${esc(r.worker_name ?? 'No worker')}</td><td class="what-${esc(r.what)}">${esc(r.what_label)}</td></tr>`,
    )
    .join('')
...
function mileageHtml(d) {
  const days = d.rows
    .map(
      (r) => `<section class="mileage-day" data-mileage-worker="${r.worker_id}" data-date="${esc(r.date)}">
    <h3>${esc(r.worker_name)} · ${esc(r.date_label)} · ${esc(r.km)} km</h3>
    <ul class="plain">${r.legs.map((l) => `<li>${esc(l.from_client)} to ${esc(l.to_client)}: ${esc(l.km)} km</li>`).join('')}</ul></section>`,
    )
    .join('')
...
export function mount(el, ctx) {
  ...
  el.innerHTML = `<div class="view-head"><h1>Reports</h1></div>
    ...
    <div class="day-tabs report-tabs" role="tablist" aria-label="Report">${KINDS.map(
      ([k, label]) => `<button type="button" role="tab" class="day-tab" data-kind="${k}" aria-selected="${k === kind}">${label}</button>`,
    ).join('')}</div>
  ...
  async function download(button) {
    ...
    try {
      let res
      try {
        res = await fetch(`/api/office/reports/${kind}.csv?from=${from}&to=${to}`, {
