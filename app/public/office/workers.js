...
export function mount(el, ctx) {
  ...
  async function load() {
    ...
    if (!alive) return
    if (r.ok) workers = r.data.workers
    else if (r.status !== 401) msg = errorText(r)
    if (!draft) list()
  }
  ...
  function list() {
    ...
    el.innerHTML = `<div class="view-head"><h1>Workers</h1><button type="button" class="btn btn-accent btn-inline" data-act="new">Add worker</button></div>
      ...
      <ul class="plain entity-list" id="worker-list">${workers
        .filter((w) => w.active)
        .map(entry)
        .join('')}</ul>
  ...
  function edit(id) {
    ...
    draft = w
      ? {
          ...
          phone: w.phone,
          zone_ids: [...w.zone_ids],
          availability: structuredClone(w.availability),
          hours: String(w.max_week_minutes / 60),
          active: w.active,
  ...
  function body() {
    ...
    for (let k = 1; k <= 7; k++) {
      availability[k] = f.querySelector(`[data-avail-on="${k}"]`).checked
        ? { start: f.querySelector(`[data-avail-start="${k}"]`).value, end: f.querySelector(`[data-avail-end="${k}"]`).value }
        : null
    }
    const hours = Number(f.querySelector('#wf-hours').value)
    return {
      name: f.querySelector('#wf-name').value,
      phone: f.querySelector('#wf-phone').value,
      zone_ids: [...f.querySelectorAll('[data-zone]:checked')].map((x) => Number(x.dataset.zone)),
      availability,
      max_week_minutes: Number.isFinite(hours) ? Math.round(hours * 60) : null,
      active: f.querySelector('#wf-active').checked,
    }
  }
  ...
  async function newLink() {
    ...
    el.querySelector('#link-msg').textContent =
      `New link made. The old link no longer works. Copy the new one and give it to ${r.data.name}.`
  }
  ...
  el.addEventListener('submit', async (e) => {
    ...
    const f = e.target
    const r = draft.id
      ? await office('PUT', `/api/office/workers/${draft.id}`, body())
      : await office('POST', '/api/office/workers', body())
    if (r.ok) {
