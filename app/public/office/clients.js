...
export function mount(el, ctx) {
  ...
  let draftPin = null
  const icon = (text, extra = '') =>
    L.divIcon({ className: `pin ${extra}`, html: `<span>${esc(text)}</span>`, iconSize: [34, 34], iconAnchor: [17, 17] })
  ...
  function drawDraftPin() {
    ...
    draftPin =
      draft?.lat != null
        ? L.marker([draft.lat, draft.lng], { title: 'Pin for this client', icon: icon('Pin', 'pin-new'), interactive: false }).addTo(map)
        : null
  }
  const pinText = () =>
    draft?.lat != null
      ? `Pin at ${draft.lat.toFixed(5)}, ${draft.lng.toFixed(5)}. Click the map again to move it.`
      : 'Click the map where the client lives to put the pin.'
  ...
  async function load() {
    ...
    if (!alive) return
    if (cr.ok) clients = cr.data.clients
    else if (cr.status !== 401) msg = errorText(cr)
  ...
  function list() {
    ...
    main.innerHTML = `<div class="view-head"><h1>Clients</h1><button type="button" class="btn btn-accent btn-inline" data-act="new">Add client</button></div>
      ...
      <ul class="plain entity-list" id="client-list">${clients
        .filter((c) => c.active)
        .map(entry)
        .join('')}</ul>
  ...
  const options = (items, selected, placeholder) =>
    `${placeholder != null ? `<option value="">${esc(placeholder)}</option>` : ''}${items
  ...
  const patternRow = (p, i) => `<div class="row-edit" data-row="pattern"><input type="hidden" data-f="id" value="${p.id ?? ''}">
    ...
    <div class="field"><label for="cf-pat-${i}-worker">Worker</label><select id="cf-pat-${i}-worker" data-f="worker_id">${options(
  ...
  function form() {
    ...
    main.innerHTML = `<form id="client-form" class="office-form" novalidate>
      ...
      <div class="field"><label for="cf-zone">Zone</label><select id="cf-zone">${options(
        a.zones.map((z) => [z.id, z.name]),
        d.zone_id,
        'Pick a zone',
      )}</select>
        <p class="field-error" data-error-for="zone_id" role="alert"></p></div>
      <div class="field"><label for="cf-funder">Who pays for the visits</label><select id="cf-funder">${options(
        a.funders.map((f) => [f.id, f.name]),
        d.funder_id,
        'Pick who pays',
      )}</select>
  ...
  function collect() {
    ...
    Object.assign(draft, {
      ...
      zone_id: val('#cf-zone'),
      funder_id: val('#cf-funder'),
      entry_notes: val('#cf-entry'),
      active: f.querySelector('#cf-active').checked,
    })
    draft.tasks = rows('task').map((r) => ({ id: num(get(r, 'id')), kind: get(r, 'kind'), detail: get(r, 'detail') }))
    draft.patterns = rows('pattern').map((r) => ({
      id: num(get(r, 'id')),
      days: [...r.querySelectorAll('[data-day]:checked')].map((x) => Number(x.dataset.day)),
      start: get(r, 'start'),
      end: get(r, 'end'),
      worker_id: num(get(r, 'worker_id')),
    }))
    draft.family_contacts = rows('family').map((r) => ({
      name: get(r, 'name'),
  ...
  const body = () => ({
    ...
    lat: draft.lat,
    lng: draft.lng,
    zone_id: num(draft.zone_id),
    entry_notes: draft.entry_notes,
    funder_id: num(draft.funder_id),
    active: draft.active,
  ...
  main.addEventListener('click', (e) => {
    ...
    if (act === 'new-link') {
      main.querySelector('#link-confirm').hidden = false
      return undefined
    }
    if (act === 'cancel-new-link') {
  ...
  async function newLink() {
    ...
    if (!r.ok) {
      showErrors(form, { ...r, data: { ...r.data, field: 'link' } })
      return
    }
    draft.family_url = r.data.family_url
    clients = clients.map((c) => (c.id === r.data.id ? r.data : c))
    main.querySelector('#link-confirm').hidden = true
    main.querySelector('#link-msg').textContent =
      'New family link made. The old link no longer works. Copy the new one and give it to the family.'
  }
  ...
  main.addEventListener('submit', async (e) => {
    ...
    btn.disabled = true
    const r = draft.id
      ? await office('PUT', `/api/office/clients/${draft.id}`, body())
      : await office('POST', '/api/office/clients', body())
    btn.disabled = false
    if (r.ok) {
