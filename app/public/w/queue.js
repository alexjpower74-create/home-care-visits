...
export function createQueue({ send, onChange = () => {}, onSent = () => {}, onKeyRefused = () => {} }) {
  ...
  const moveToRefused = (seq, data) =>
    withItem(['queue', 'refused'], seq, (tx, item) => {
      tx.objectStore('queue').delete(seq)
      tx.objectStore('refused').put({
        ...item,
        error: data?.error || 'The office did not accept this.',
        code: data?.code ?? null,
        refused_at: Date.now(),
      })
    })
  ...
  async function sendItem(item, answers) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (s === 400 || s === 404 || s === 409) {
        await moveToRefused(item.seq, res.data)
        return 'refused'
      }
  ...
  function withLock(fn) {
    ...
    return navigator.locks.request('hcv-send', opts, async (lock) => {
      if (lock) await fn()
    })
  }
  ...
  return {
    ...
    async removeHeld(seq) {
      await withItem(['queue'], seq, (tx, item) => {
        if (item.key_refused) tx.objectStore('queue').delete(seq)
      })
      onChange()
    },
  }
