...
addEventListener('pageshow', (e) => {
  if (e.persisted) leaving = new AbortController()
})
const loadSignal = () => {
  const c = new AbortController()
  const stop = () => c.abort()
  if (leaving.signal.aborted) stop()
  else leaving.signal.addEventListener('abort', stop, { once: true })
...
function rememberRefusals(answers = []) {
  for (const { item, data } of answers) {
    ...
    ls((s) =>
      s.setItem(
        NOTICE_PREFIX + item.event.visit_id,
...
function withQueued(answer, queued, date) {
  ...
  for (const i of queued) {
    ...
    extra.push({
      ...
      client_initials: initials(i.client_name),
      address: '',
      lat: null,
      lng: null,
      entry_notes: '',
  ...
  for (let d = EARLIER_DAYS; d >= 1; d--) {
    ...
    if (online) {
      try {
        ...
        /* no signal: the saved list and the queue stand in */
      }
    }
...
function view(v) {
  ...
  const cin = v.check_in
    ? { label: v.check_in.at_label, where: v.check_in.location_label, saved: false }
    : qIn
      ? { label: timeLabel(qIn.event.at, tz()), where: 'saved on this phone', saved: true }
      : null
  const cout = v.check_out
    ? { label: v.check_out.at_label, saved: false }
    : qOut
      ? { label: timeLabel(qOut.event.at, tz()), saved: true }
      : null
...
function statusLine(x) {
  if (x.status === 'done')
    return `Done ${x.cin?.label ?? ''} – ${x.cout.label}${x.cin?.saved || x.cout.saved ? ' · saved on this phone' : ''}`
...
const navigateHref = (v) =>
  isApple()
    ? `https://maps.apple.com/?daddr=${v.lat},${v.lng}&dirflg=d`
    : `https://www.google.com/maps/dir/?api=1&destination=${v.lat},${v.lng}`
const TICK =
  '<svg width="18" height="18" viewBox="0 0 20 20" aria-hidden="true"><path d="M4 10.5l4 4 8-9" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"/></svg>'
...
function bodyHtml(x) {
  ...
  if (x.status === 'todo' || x.status === 'refused') {
    ...
    const nav =
      v.lat != null
        ? `<a class="btn btn-outline" href="${esc(navigateHref(v))}" target="_blank" rel="noopener noreferrer">Navigate</a>`
        : ''
  ...
  if (x.status === 'in') {
    ...
    const tasks = v.tasks
      .map(
        (
          t,
        ) => `<label class="task"><input type="checkbox" data-act="task" data-visit="${v.id}" data-task="${t.id}"${d.done[t.id] ? ' checked' : ''}>
      <span class="task-text"><span class="task-label">${esc(t.label)}</span>${t.detail ? `<span class="task-detail">${esc(t.detail)}</span>` : ''}${t.kind === 'medication_reminder' ? '<span class="task-note">Reminder only. This app doesn\'t record medication given.</span>' : ''}</span></label>`,
      )
      .join('')
  ...
  if (x.status === 'done') {
    const ticks = x.tasksDone.length
      ? `<ul class="ticks">${x.tasksDone.map((t) => `<li>${TICK}<span>${esc(t.label)}</span></li>`).join('')}</ul>`
      : '<p class="muted">No tasks ticked.</p>'
...
function refusedHtml() {
  ...
  return `<section class="panel panel-bad" aria-labelledby="refused-h"><h2 id="refused-h">Not accepted by the office</h2><ul class="plain">${S.refused
    .map(
      (i) => `<li class="refused-item">
      ...
      <button type="button" class="btn btn-outline" data-act="remove-refused" data-seq="${i.seq}">Remove</button></li>`,
    )
    .join('')}</ul></section>`
}
...
function heldHtml() {
  ...
  return `<section class="panel panel-bad" aria-labelledby="held-h"><h2 id="held-h">Saved under an old link for another worker</h2><ul class="plain">${held
    .map(
      (
        i,
      ) => `<li><p><strong>${kindWord(i.event.kind)}</strong> for ${esc(i.client_name)}, tapped at ${esc(timeLabel(i.event.at, i.tz || tz()))}</p>
      ...
      <button type="button" class="btn btn-outline" data-act="remove-held" data-seq="${i.seq}">Remove</button></li>`,
    )
    .join('')}</ul></section>`
}
...
function mainHtml() {
  ...
  if (!key)
    out.push(
      '<div class="notice notice-bad" role="alert">This page needs the link the office gave you. Ask the office for your link.</div>',
    )
  else if (refusedKey)
    out.push(`<div class="notice notice-bad" role="alert">This link doesn't work any more. Ask the office for a new one.</div>`)
    ...
    if (!S.openChosen) {
      S.openId =
        (
          [...earlier.flatMap((g) => g.xs), ...xs].find((x) => x.status === 'in') ||
          xs.find((x) => x.status === 'todo' || x.status === 'refused')
        )?.v.id ?? null
    }
...
function stripState() {
  ...
  if (!navigator.onLine) {
    state = 'offline'
    text = n
      ? `No signal. ${n} saved on this phone. They send when signal comes back and keep the time you tapped.`
      : 'No signal. Nothing is waiting to send.'
  } else if (!n) {
...
function patchChildren(parent, next) {
  ...
  for (const want of [...next.childNodes]) {
    ...
    if (!match && keyOf(want)) {
      for (let c = cur?.nextSibling; c; c = c.nextSibling)
        if (sameKind(c, want)) {
          match = c
          break
        }
...
function render() {
  ...
  if (setHtml($('main'), mainHtml()) && focusId && $(focusId)) {
    $(focusId).focus()
    $(focusId).setSelectionRange(...sel)
  }
  ...
  setHtml(
    $('foot'),
    `${mileage ? `<p>${esc(mileage.km)} km between visits today (straight line)</p>` : ''}${phone ? `<p>Office: <a href="${telHref(phone)}">${esc(phone)}</a></p>` : ''}`,
  )

  setHtml($('sheet-root'), sheetHtml())
  if (S.sheetOpened && S.sheetFor) {
    S.sheetOpened = false
    $('sheet-title')?.focus()
  }
}
...
function metaFor(v) {
  ...
  return {
    key,
    worker_id: a?.worker.id ?? held?.worker_id,
    client_name: v.client_name,
    time_label: v.time_label,
    visit_date: v.date,
    office_phone: a?.agency.office_phone ?? held?.office_phone ?? '',
    tz: tz(),
  }
}
...
document.addEventListener('click', (e) => {
  ...
  switch (t.dataset.act) {
    ...
    case 'confirm-check-out':
      confirmCheckOut()
      break
    case 'check-out-without-note':
      confirmCheckOut({ withoutNote: true })
      break
    case 'dismiss-notice':
      ls((s) => s.removeItem(NOTICE_PREFIX + id))
      render()
      break
    case 'dismiss-refused':
      ...
      break
    case 'remove-refused':
      queue.removeRefused(Number(t.dataset.seq))
      break
    case 'remove-held':
      queue.removeHeld(Number(t.dataset.seq))
      break
    default:
      break
  }
