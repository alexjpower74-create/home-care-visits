...
const TICK = '<svg width="20" height="20" viewBox="0 0 20 20" aria-hidden="true"><path d="M4 10.5l4 4 8-9" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function render(d) {
  ...
  const days = d.week.days.map(day => `<section class="fam-day"><h3${day.date === d.today.date ? ' class="is-today"' : ''}>${esc(day.date_label)}${day.date === d.today.date ? ' (today)' : ''}</h3>${
    day.visits.length ? `<ul class="plain">${day.visits.map(v => `<li><span>${esc(v.time_label)}</span><span>${esc(v.status_label)}</span></li>`).join('')}</ul>` : '<p class="muted" style="margin:0">No visits</p>'
  }</section>`).join('');
...
async function badLink(message) {
  ...
  try { const r = await getAgency(); if (r.ok) header(r.data); } catch { /* keep the generic header */ }
}
...
async function load() {
  ...
  try {
    ...
    if (r.status === 200) { last = r.data; render(r.data); return; }
    ...
    if (!last) {
      $('main').innerHTML = `<div class="notice" role="alert" style="margin-top:24px">${esc(e instanceof TypeError ? "Can't reach the agency right now. This page tries again every minute." : e.message)}</div>`;
    } else if ($('updated')) {
