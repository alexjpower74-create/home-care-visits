...
const hm = h => { const [H, M] = h.split(':').map(Number); return `${H % 12 || 12}:${String(M).padStart(2, '0')} ${H < 12 ? 'AM' : 'PM'}`; };
const dayNum = date => Math.round(Date.parse(`${date}T00:00:00Z`) / 86400000);
const agency = () => ({ name: DATA.agency.name, sample: DATA.agency.name.includes('SAMPLE'), office_phone: DATA.agency.office_phone, timezone: TZ });
...
function ensureDate(date) {
  ...
  for (const c of DATA.clients) c.patterns.forEach((p, i) => {
    if (!p.days.includes(wd)) return;
    const id = c.id * 1000000 + (i + 1) * 100000 + (dayNum(date) % 100000);
    const v = { id, client: c, worker_id: p.worker_id, date, start: p.start, end: p.end,
      starts_at: localToUtcMs(date, p.start), ends_at: localToUtcMs(date, p.end), cancelled: false, cancel_reason: null,
...
function seedDone(v) {
  ...
  store(v, { id: `seed-in-${v.id}`, kind: 'check_in', at: iso(inAt), location: { lat: v.client.lat, lng: v.client.lng }, worker_id: v.worker_id });
  const note = v.client.id === 8 ? 'Walter ate a good breakfast and we talked about his garden. (SAMPLE)' : null;
  store(v, { id: `seed-out-${v.id}`, kind: 'check_out', at: iso(v.ends_at + ((v.id % 7) - 3) * 60000), worker_id: v.worker_id,
    tasks: v.client.tasks.map(t => ({ task_id: t.id, kind: t.kind, label: t.label, done: true })), note });
...
function store(v, e, source = 'phone') {
  ...
  if (e.kind === 'check_in') {
    ...
    ev.location_label = d == null ? 'Location not shared'
      : d <= 250 ? 'Within 250 m of the client'
...
function workerVisit(v) {
  const c = v.client;
  return { id: v.id, client_id: c.id, client_name: c.name, client_initials: initials(c.name), address: c.address, lat: c.lat, lng: c.lng,
    entry_notes: c.entry_notes, tasks: c.tasks, date: v.date, start: v.start, end: v.end, time_label: `${hm(v.start)} – ${hm(v.end)}`,
    starts_at: iso(v.starts_at), ends_at: iso(v.ends_at), cancelled: v.cancelled, cancel_reason: v.cancel_reason,
    check_in: eventView(v.check_in), check_out: eventView(v.check_out),
    tasks_done: v.check_out ? v.check_out.tasks : [], note: v.note ? { text: v.note.text } : null };
}
...
function workerVisits(w, url) {
  ...
  ensureDate(date);
  const mine = [...visits.values()].filter(v => v.date === date && v.worker_id === w.id).sort((a, b) => a.starts_at - b.starts_at || a.id - b.id);
  const ins = mine.filter(v => v.check_in).sort((a, b) => a.check_in.at.localeCompare(b.check_in.at) || a.id - b.id);
  let metres = 0;
  for (let i = 1; i < ins.length; i++) metres += haversineM(ins[i - 1].client.lat, ins[i - 1].client.lng, ins[i].client.lat, ins[i].client.lng);
  return ok({ worker: { id: w.id, name: w.name, initials: initials(w.name) }, agency: agency(), date, date_label: dateLabelOf(date),
    server_now: new Date().toISOString(), visits: mine.map(workerVisit),
...
function postEvent(w, b) {
  ...
  if (b.kind === 'check_in') {
    if (v.check_in) return err(409, 'already_checked_in', `This visit already has a check-in at ${v.check_in.at_label}.`, { event: eventView(v.check_in) });
    const e = store(v, { id: b.id.toLowerCase(), kind: 'check_in', at: iso(Date.parse(b.at)), location: b.location || null, worker_id: w.id });
    return ok({ event: eventView(e), visit: workerVisit(v) }, 201);
  }
  if (!v.check_in) return err(409, 'bad_state', 'Check in before you check out.');
  if (v.check_out) return err(409, 'already_checked_out', `This visit already has a check-out at ${v.check_out.at_label}.`, { event: eventView(v.check_out) });
...
function familyVisit(v, today) {
  ...
  const inL = v.check_in?.at_label ?? null, outL = v.check_out?.at_label ?? null;
  let status, label;
  ...
  return { time_label: `${hm(v.start)} – ${hm(v.end)}`, worker_first_name: w ? w.name.split(' ')[0] : null, status, status_label: label,
    arrived_label: inL, left_label: outL, tasks_done: v.check_out ? v.check_out.tasks.filter(t => t.done).map(t => t.label) : [],
...
function family(key) {
  ...
  const forDate = date => { ensureDate(date); return [...visits.values()].filter(v => v.client.id === c.id && v.date === date).sort((a, b) => a.starts_at - b.starts_at).map(v => familyVisit(v, today)); };
  const days = Array.from({ length: 7 }, (_, i) => addDays(monday, i)).map(date => ({ date, date_label: dateLabelOf(date), visits: forDate(date) }));
  return ok({ agency: { name: DATA.agency.name, sample: true, office_phone: DATA.agency.office_phone },
...
globalThis.hcvMock = {
  officeFixTimes(clientName) {
    ...
    store(v, { id: `office-in-${v.id}`, kind: 'check_in', at: iso(Date.now() - 20 * 60000), location: null, worker_id: v.worker_id }, 'office');
