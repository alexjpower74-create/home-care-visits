...
function expectRetryGap(from, to, failures, label, slack = TICK_MS) {
  ...
  expect(
    gap,
    `${label}: ${gap} ms of page time after ${failures} failure(s), not before the ${backoff / 1000} s backoff`,
  ).toBeGreaterThanOrEqual(backoff - 1000)
  expect(gap, `${label}: ${gap} ms of page time, within the ${backoff / 1000} s backoff plus ${slack / 1000} s`).toBeLessThanOrEqual(
    backoff + slack,
  )
}
...
const WEBKIT_RELOAD =
  'webkit: page.reload() with the context offline fails in Playwright\'s WebKit with "WebKit encountered an internal ' +
...
const WEBKIT_SW_ROUTE =
  "webkit: Playwright's WebKit neither reloads a page with the context offline nor routes a service worker's " +
  'own fetches, so a test built on either runs in Chromium only'

test.use({ permissions: ['geolocation'] })

const controlledBySw = (page) =>
  page.evaluate(async () => {
...
test("with no signal after midnight, a reload still shows yesterday's open visit and checks it out", async ({
  page,
  context,
  request,
  seed,
...
test("a Wi-Fi login page answering the worker page's files never replaces them in the cache", async ({
  page,
  context,
  request,
  seed,
  ...
  let served = 0
  await context.route(
    (url) => url.hostname === '127.0.0.1' && FILES.includes(url.pathname),
    (route) => {
      ...
      return route.fulfill({
        ...
        body: '<!doctype html><title>Free Wi-Fi</title><p>Log in to continue.</p>',
      })
    },
  )
  await page.reload()
...
test('no signal: check-in and check-out are saved, survive a reload, and send later with the times tapped', async ({
  page,
  context,
  request,
  seed,
  ...
  if (browserName === 'chromium') {
    await expect(page.locator('#strip-text')).toHaveText(
      'No signal. 2 saved on this phone. They send when signal comes back and keep the time you tapped.',
    )
  }
  ...
  if (browserName === 'webkit') {
    test
      .info()
      .annotations.push({ type: 'skipped step', description: `${WEBKIT_RELOAD} (service worker controlling the page: ${controlled})` })
  } else {
    await page.reload()
    await expect(card.locator('.visit-status')).toHaveText('Done 10:30 AM – 12:02 PM · saved on this phone')
    await expect(page.locator('#strip-text')).toHaveText(/(^|\. )2 saved on this phone\./)
...
test.describe('with the service worker blocked', () => {
  ...
  test('a 500 when signal comes back leaves both queued; they send on the next try', async ({ page, context, request, seed }) => {
    ...
    await expect(page.locator('#strip-text'), 'the check-in is still saved on the phone after a failed send').toHaveText(
  ...
  test('a check-out the office already has lands in "Not accepted by the office" with the server\'s words', async ({
    page,
    context,
    request,
    seed,
    ...
    const other = await api(request, 'POST', '/api/worker/events', {
    ...
    expect(
      tries.map((x) => x.ok),
      'no signal, the dropped send, then the answer',
    ...
    expect(
      events.filter((e) => e.kind === 'check_out'),
      'still exactly one check-out',
    ).toHaveLength(1)
  })

  test("a 200 that is not the Worker's answer (a Wi-Fi login page) keeps the event queued; it then lands exactly once", async ({
    page,
    context,
    request,
    seed,
  ...
  test('a refused link takes the saved lists and drafts off the phone and keeps the queue', async ({ page, context, request, seed }) => {
    ...
    expect(
      before.some((k) => k.startsWith('hcv:visits:')),
      'a saved list is on the phone',
    ).toBe(true)
  ...
  test('a check-in the office already set with Fix times stays on its card: "Not accepted by the office"', async ({
    page,
    context,
    request,
    seed,
    ...
    await page.route('**/api/worker/events', async (route) => {
      ...
      if (dropped === 0) {
        ...
        tries.push({ t, ok: false })
        return route.abort('connectionreset')
      } // one dropped send after the signal returns
      tries.push({ t, ok: true })
      return route.continue()
    })
    await tap(page, card.getByRole('button', { name: 'Check in' }), 'Check in (no signal)')
    ...
    const fix = await api(request, 'PUT', `/api/office/visits/${visit.id}/times`, {
      token,
    ...
    await expect(card.locator('.visit-status'), "the office's check-in is the visit's").toHaveText(
      'Checked in 10:10 AM · Location not shared',
    )
  ...
  test('the saved list and its Check in are on screen within 1 s when the visits request never answers', async ({
    page,
    context,
    request,
    seed,
    ...
    await page.reload()
    await expect(card.getByRole('button', { name: 'Check in' }), 'the saved list, without waiting for the network').toBeVisible({
      timeout: 1000,
    })
    await expect(card.locator('.visit-name')).toHaveText(visit.client_name)
    await expect(page.locator('.notice-saved')).toHaveText('Saved list from 10:30 AM')
