...
// copy and runs the spec again: the control exits 0 only if that second run goes red. The copies are served on 7906
// (inspector 7916). Output is appended to app/tests/negative-control.log. The shipped code has no switch for any break.
...
const PORT = '7906';
// No machine paths in the committed log: the repository roots (this worktree, and the checkout node_modules resolves to)
// and their file:// forms become <repo>; the home folder becomes <home> as a last guard.
...
const SKIP = /[\\/](\.state-[^\\/]*|\.wrangler|node_modules|results|\.logs)([\\/]|$)/;
...
// Each control keeps Playwright's artifacts inside its own copy: a run empties its output folder when it starts, and a control
// sharing app/tests/results with the suite deleted the other run's traces (two voided controls, 2026-09-14).
function runSpec(copy, args) {
  const cli = path.join(APP, 'node_modules', '@playwright', 'test', 'cli.js');
  const r = spawnSync(process.execPath, [cli, 'test', ...args, '--output', path.join(copy, 'results')], {
...
const redLines = out => {
  ...
  return (at < 0 ? lines.slice(-40) : lines.slice(at, at + 30)).join('\n');
};
...
export function control({ name, what, args, breakIt }) {
  ...
  appendFileSync(LOG, scrub([
    `== ${stamp} ${name}: ${what}`,
    `   spec: playwright test ${args.join(' ')} on ${PORT} (inspector 7916), copy in app/.negative/${name}`,
