...
test("add a client by typing and clicking the map, with a task and a Mon/Wed pattern: in the list, on the map, on Alex B.'s row", async ({
  page,
  context,
  seed,
}, testInfo) => {
  ...
  expect((await saved.json()).patterns[0], 'the pattern as stored').toMatchObject({
    days: [1, 3],
    start: '13:00',
    end: '14:00',
    worker_id: alex.id,
  ...
  for (const date of [DAY, addDays(DAY, 2)]) {
    if (testInfo.project.name.endsWith('1280')) {
      await expect(
        page.locator(`.cell[data-worker-id="${alex.id}"][data-date="${date}"] .chip`, { hasText: name }),
        `a chip on Alex B.'s row on ${date}`,
      ).toHaveCount(1)
      ...
      await expect(
        page.locator(`.wgroup[data-worker-id="${alex.id}"] .vcard`, { hasText: name }),
        `a visit under Alex B. on ${date}`,
      ).toHaveCount(1)
    }
  }
})

test('the Clients map: the OpenFreeMap attribution and its three links, a pin placed by clicking, only the routed tiles leave 127.0.0.1', async ({
  page,
  context,
  guarded,
}, testInfo) => {
  ...
  await expect
    .poll(() => [...new Set(guarded.tiles)].filter((t) => !t.endsWith('.pbf')).sort(), {
      message: 'the style and its TileJSON came from the routed fixtures',
    })
...
test('an office PC without WebGL: the Clients map shows on its plain background, a pin places by clicking, the attribution and links are there, no page error @desktop', async ({
  page,
  context,
}) => {
...
test('a task "Give her pills" shows the medication message by the tasks field', async ({ page, context }) => {
  ...
  await expect(page.locator('[data-error-for="tasks"]')).toHaveText(
    'This app records medication reminders only, not medication given. Reword this task.',
  )
})
...
test('a deactivated worker stays under Inactive, their earlier visit keeps them when saved, and the week shows their row', async ({
  page,
  context,
  request,
  seed,
  ...
  const visit = await oneOffVisit(request, token, {
    client_id: byName(seed.clients, 'George N. (SAMPLE)').id,
    worker_id: terry.id,
    date: DAY,
    start: '08:00',
    end: '09:00',
  ...
  if (testInfo.project.name.endsWith('1280')) {
    ...
    await tap(
      page,
      page.locator(`.cell[data-worker-id="${terry.id}"][data-date="${DAY}"] .chip[data-visit-id="${visit.id}"]`),
      'the 8:00 visit',
    )
  } else {
