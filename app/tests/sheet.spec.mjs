...
test('a check-out typed earlier than the check-in is refused unless "The check-out was after midnight" is ticked', async ({
  page,
  context,
  request,
  seed,
...
test('an 11:00-11:55 PM visit checked in at 12:10 AM: a 12:50 AM check-out is fixed on the next day, 40 minutes worked', async ({
  page,
  context,
  request,
  seed,
...
test('a fix time in the spring-forward gap shows the words and is not sent', async ({ page, context, request, seed }) => {
  ...
  let puts = 0
  page.on('request', (r) => {
    if (r.method() === 'PUT' && r.url().endsWith(`/api/office/visits/${visit.id}/times`)) puts += 1
  })
