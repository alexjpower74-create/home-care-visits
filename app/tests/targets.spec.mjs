...
test('every worker-page button is at least 48 px (Check in / Check out 56 px) and hit-tests to itself at 390 @phone', async ({ page, context, seed }) => {
...
test('the SAMPLE badge is visible on /, /office/, /w/ and /f/', async ({ page, context, seed }) => {
  await setNow(page, context, NOW);
  for (const [url, label] of [['/', 'landing'], ['/office/', 'office'], [pathOf(seed.workers[0].worker_url), 'worker'], [pathOf(seed.clients[0].family_url), 'family']]) {
    await page.goto(url);
...
test('no horizontal scroll at 390 on any page @phone', async ({ page, context, seed }) => {
  ...
  for (const [url, label] of [['/', 'landing'], [pathOf(byName(seed.workers, 'Sam R. (SAMPLE)').worker_url), 'worker'], [pathOf(seed.clients[0].family_url), 'family']]) {
    await page.goto(url);
...
test('Check in, Check out and the late and missed rows meet 4.5 : 1', async ({ page, context, seed }) => {
  const ratio = async (locator, label) => {
    const [fg, bg] = await locator.evaluate(el => {
      ...
      while (n && /rgba\(0, 0, 0, 0\)|transparent/.test(back)) { back = getComputedStyle(n).backgroundColor; n = n.parentElement; }
