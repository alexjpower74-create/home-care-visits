...
async function context(browser, opts, problems) {
  const ctx = await browser.newContext({ ...opts, permissions: ['geolocation'], geolocation: { latitude: ruby.lat, longitude: ruby.lng, accuracy: 15 } });
...
async function project(name, engine, opts) {
  ...
  try {
    ...
    await page.getByText(/^Arrived \d+:\d\d AM, left \d+:\d\d AM$/).first().waitFor(WAIT);
