...
const CLIPBOARD_WEBKIT =
  "webkit: Playwright's WebKit can't grant clipboard-read, so the clipboard's text can't be read back; " +
...
test('New link on a worker: after the inline confirm the old link stops working and the new one works', async ({ page, context, seed }) => {
  ...
  await expect(page.locator('#link-msg')).toHaveText(
    'New link made. The old link no longer works. Copy the new one and give it to Sam R. (SAMPLE).',
  )
...
test('New link on an inactive worker, opened from Inactive', async ({ page, context, request, seed }) => {
  ...
  const off = await api(request, 'PUT', `/api/office/workers/${terry.id}`, {
    token,
    data: {
      name: w.name,
      phone: w.phone,
