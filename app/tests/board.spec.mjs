...
import { test, expect, signIn, api, officeToken, oneOffVisit, byName, rgbOfHex, randomUUID, iso, localToUtcMs, DAY, MIN } from './helpers.mjs';

test('the Today board turns late at 15:00 and missed at 30:00 on the page clock, and done after a check-in', async ({ page, context, request, seed }) => {
  test.setTimeout(180_000);
  const token = await officeToken(request);
  const sam = byName(seed.workers, 'Sam R. (SAMPLE)');
  const start = localToUtcMs(DAY, '09:00');
  const visit = await oneOffVisit(request, token, { client_id: byName(seed.clients, 'Ron K. (SAMPLE)').id, worker_id: sam.id, date: DAY, start: '09:00', end: '10:00' });
  const off = await oneOffVisit(request, token, { client_id: byName(seed.clients, 'Irene C. (SAMPLE)').id, worker_id: sam.id, date: DAY, start: '09:00', end: '10:00' });
  const c = await api(request, 'POST', `/api/office/visits/${off.id}/cancel`, { token, data: { reason: 'Client away this week (SAMPLE)', version: off.version } });
  ...
  const ev = await api(request, 'POST', '/api/worker/events', { headers: { 'X-Worker-Key': sam.key }, now: start + 31 * MIN,
