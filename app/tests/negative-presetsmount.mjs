...
process.exit(control({
  ...
  breakIt: copy => {
    ...
    replaceOnce(file, "  let [from, to] = presetRange('this-week');",
    ...
    replaceOnce(file, '      [from, to] = presetRange(b.dataset.preset);', '      [from, to] = MOUNT_PRESETS[b.dataset.preset]; // NEGATIVE CONTROL (j)');
  },
}));
