...
test("restoring a visit removed by a pattern change, after the phone checked in, shows the Worker's words (clarification 14)", async ({
  page,
  context,
  request,
  seed,
  ...
  const moved = await api(request, 'PUT', `/api/office/clients/${walterId}`, {
    token,
    data: {
      name: c.name,
      address: c.address,
  ...
  const late = await api(request, 'POST', '/api/worker/events', {
    headers: { 'X-Worker-Key': alex.key },
    now: localToUtcMs(TUE, '09:30'),
...
test("a mouse drag of Terry O.'s Saturday chip onto Jo W. clears the travel gap, and a reload keeps it @desktop", async ({
  page,
  context,
  request,
}) => {
  ...
  await tab(page, 'Week')
  await expect(
    page.locator(`.cell[data-worker-id="${jo.id}"][data-date="${SAT}"] .chip[data-visit-id="${george.id}"]`),
    'kept after a reload',
  ).toBeVisible()
...
test('dragging a visit onto a worker already booked then shows Double-booked with both chips edged @desktop', async ({
  page,
  context,
  request,
}) => {
  ...
  await drag(
    page,
    page.locator(`.cell[data-worker-id="${alex.id}"][data-date="${MON}"] .chip[data-visit-id="${walter.id}"]`),
    page.locator(`.cell[data-worker-id="${sam.id}"][data-date="${MON}"]`),
    'Walter G. to Sam R.',
  )
  expect((await put).status()).toBe(200)
  ...
  await expect(conflict.locator('.conflict-msg')).toHaveText(
    /^Sam R\. \(SAMPLE\) is booked for (Bill S\. \(SAMPLE\) and Walter G\. \(SAMPLE\)|Walter G\. \(SAMPLE\) and Bill S\. \(SAMPLE\)) at the same time on Mon Sep 14\.$/,
  )
...
test('a stale edit shows "This visit was changed on another screen. Reload and try again."', async ({
  page,
  context,
  request,
}, testInfo) => {
  ...
  const other = await api(request, 'PUT', `/api/office/visits/${george.id}`, {
    token,
  ...
  let puts = 0
  page.on('request', (r) => {
    if (r.method() === 'PUT' && r.url().endsWith(`/api/office/visits/${george.id}`)) puts += 1
  })
  const put = page.waitForResponse((r) => r.url().endsWith(`/api/office/visits/${george.id}`) && r.request().method() === 'PUT')
