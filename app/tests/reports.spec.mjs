...
test('presets read the page clock when pressed: three days on without a reload, "Last week" is the new last week', async ({
  page,
  context,
}) => {
  ...
  await expect
    .poll(() => inputs(page), { message: 'the week before Tue Sep 15, not before Sat Sep 12' })
...
test('Download CSV uses the From and To typed at the click, and shows that period first', async ({ page, context, request }) => {
  ...
  const direct = await request.get('/api/office/reports/payroll.csv?from=2026-09-01&to=2026-09-10', {
...
test("Billing prints the Worker's scheduled hours: three 20-minute clients are 0.33 each and 1.00 in total", async ({
  page,
  context,
  request,
  seed,
  ...
  for (const [i, name] of names.entries()) {
    ...
    const v = await oneOffVisit(request, token, {
      client_id: byName(seed.clients, name).id,
      worker_id: sam.id,
      date: DAY,
    ...
    for (const [kind, at, extra] of [
      ['check_in', inAt, { location: null }],
      ['check_out', inAt + 20 * 60_000, { tasks: [] }],
    ]) {
      const r = await api(request, 'POST', '/api/worker/events', {
  ...
  const billing = (
    await api(request, 'GET', `/api/office/reports/billing?from=${DAY}&to=${DAY}`, { token, now: localToUtcMs(DAY, '15:00') })
  ).body
  const clients = billing.funders.flatMap((f) => f.clients.map((c) => ({ ...c, funder_id: f.funder_id })))
  expect(
    clients.map((c) => c.scheduled_hours),
    'each 20-minute client',
  ...
  for (const c of clients) {
    await expect(
      page.locator(`#billing-table tr.row-sub[data-funder-id="${c.funder_id}"][data-client-id="${c.client_id}"] .scheduled`),
    ).toHaveText('0.33')
  }
...
async function checkInOnPhone(page, context, worker, visit, at) {
  ...
  if (!(await card.getByRole('button', { name: 'Check in' }).isVisible()))
    await tap(page, card.locator('.visit-head'), `open ${visit.client_name}`)
...
test("Payroll shows the Worker's exact hours after two phone journeys and a Fix times; the CSV is the Worker's bytes; Billing, Missed, Mileage", async ({
  page,
  context,
  request,
  seed,
  ...
  const report = async (kind) =>
    (await api(request, 'GET', `/api/office/reports/${kind}?from=${WEEK[0]}&to=${WEEK[1]}`, { token, now: OFFICE_AT })).body
  await signIn(page)
  await tab(page, 'Reports')
  await expect(page.locator('#rp-from')).toHaveValue(WEEK[0])
  await expect(page.locator('#rp-to')).toHaveValue(WEEK[1])
  await expect(page.locator(`#payroll-incomplete [data-visit-id="${margaret.id}"]`)).toHaveText(
    'Jo W. (SAMPLE) · Margaret P. (SAMPLE) · Mon Sep 14, checked in 8:30 AM',
  )
  ...
  expect(
    payroll.rows.map((r) => r.hours),
    'each row rounds up on its own',
  ).toEqual(['1.01', '1.01'])
  for (const r of payroll.rows) {
    ...
    for (const c of r.clients)
      await expect(
        page.locator(`#payroll-table tr.row-sub[data-worker-id="${r.worker_id}"][data-client-id="${c.client_id}"] .hours`),
      ).toHaveText(c.hours)
  }
  await expect(page.locator('#payroll-total .hours'), "the Worker's total, not the rounded rows added up (2.02)").toHaveText(
    payroll.total.hours,
  )
  ...
  const direct = await request.get(`/api/office/reports/payroll.csv?from=${WEEK[0]}&to=${WEEK[1]}`, {
    headers: { Authorization: `Bearer ${token}`, 'X-Test-Now': iso(OFFICE_AT) },
  })
  ...
  for (const f of billing.funders) {
    ...
    for (const c of f.clients) {
      await expect(
        page.locator(`#billing-table tr.row-sub[data-funder-id="${f.funder_id}"][data-client-id="${c.client_id}"] .scheduled`),
      ).toHaveText(c.scheduled_hours)
    }
  }
