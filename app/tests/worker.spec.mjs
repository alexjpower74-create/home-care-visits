...
test('Sam R.: visits in order, Navigate, check in near the client, tasks and note, check out 47 minutes later', async ({
  page,
  context,
  request,
  seed,
  ...
  expect(
    visits.map((v) => v.starts_at),
    'the API answers in start order',
  ).toEqual([...visits.map((v) => v.starts_at)].sort())
  const [first] = visits
...
test('a visit checked in from a lost phone three days ago opens on a clean phone from open_dates, and checks out', async ({
  page,
  context,
  request,
  seed,
  ...
  const bill = (await api(request, 'GET', `/api/worker/visits?date=${friday}`, { headers: { 'X-Worker-Key': sam.key } })).body.visits.find(
    (v) => v.client_name === 'Bill S. (SAMPLE)',
  )
  const inAt = localToUtcMs(friday, '09:05')
  const checkIn = await api(request, 'POST', '/api/worker/events', {
...
test('a visit still open after midnight shows under "Still open from yesterday" and checks out with the right hours', async ({
  page,
  context,
  request,
  seed,
  ...
  const listOf = (date) =>
    page.waitForResponse(
      (r) => new URL(r.url()).pathname === '/api/worker/visits' && (new URL(r.url()).searchParams.get('date') ?? '') === date,
    )
...
test.describe('with the service worker blocked', () => {
  ...
  test('a note refused on its way out: the check-out lands and the card says "The note wasn\'t saved: …"', async ({
    page,
    context,
    request,
    seed,
  ...
  test('"Check out without the note" next to the disabled "Yes, check out" checks out at that tap, with no note', async ({
    page,
    context,
    request,
    seed,
  ...
  test('a refused note reported only by the resend (the first answer was lost) still says "The note wasn\'t saved"', async ({
    page,
    context,
    request,
    seed,
  ...
  test('a refused check-in with no other check-in offers "Check in again"; the new one lands and the old reads as history', async ({
    page,
    context,
    request,
    seed,
    ...
    const todayList = page.waitForResponse(
      (r) => new URL(r.url()).pathname === '/api/worker/visits' && !new URL(r.url()).searchParams.get('date'),
    )
    await page.reload()
  ...
  test('a check-in still queued across midnight shows under "Still open from yesterday", then both land', async ({
    page,
    context,
    request,
    seed,
    ...
    await setNow(page, context, outAt)
    const isList = (date) => (r) =>
      new URL(r.url()).pathname === '/api/worker/visits' && (new URL(r.url()).searchParams.get('date') ?? '') === date
    let release
    ...
    await open.evaluate((el) => {
      el.drawnFromThePhone = true
    }) // a property, not an attribute: the markup stays the same
    release()
    expect((await todayList).status()).toBe(200)
  ...
  test('a check-in queued before midnight under an old link still opens "Still open from yesterday" on the new link', async ({
    page,
    context,
    request,
    seed,
  ...
  test('a Saturday check-in still open on Monday shows under "Still open from Sat Sep 12", even when today\'s list fails', async ({
    page,
    context,
    request,
    seed,
    ...
    const george = (
      await api(request, 'GET', `/api/worker/visits?date=${sat}`, { headers: { 'X-Worker-Key': terry.key } })
    ).body.visits.find((v) => v.client_name === 'George N. (SAMPLE)')
    let signal = false
    await page.route('**/api/worker/events', (route) => (signal ? route.continue() : route.abort('internetdisconnected')))
    ...
    await page.route('**/api/worker/visits*', (route) =>
      new URL(route.request().url()).searchParams.get('date')
        ? route.continue()
        : route.fulfill({
            status: 500,
    ...
    const satList = page.waitForResponse(
      (r) => new URL(r.url()).pathname === '/api/worker/visits' && new URL(r.url()).searchParams.get('date') === sat,
    )
    await page.reload()
...
test.describe('moved by the office while a check-in waited', () => {
  ...
  test('a visit Sam checked in to with no signal, moved to Jo W. before it landed, stays on Sam\'s list with "Moved to another worker by the office" and checks out', async ({
    page,
    context,
    request,
    seed,
    ...
    const token = await officeToken(request)
    const v = await officeVisit(request, token, first.id)
    const moved = await api(request, 'PUT', `/api/office/visits/${first.id}`, {
      token,
      data: { worker_id: jo.id, date: v.date, start: v.start, end: v.end, version: v.version },
    })
    ...
    const [timeBottom, lineTop] = await again.evaluate((el) => [
      el.querySelector('.visit-time').getBoundingClientRect().bottom,
      el.querySelector('.visit-moved').getBoundingClientRect().top,
    ])
    ...
    await setNow(page, context, OUT)
    if (!(await again.getByRole('button', { name: 'Check out' }).isVisible()))
      await tap(page, again.locator('.visit-head'), 'open the moved visit')
