...
test('office: every screen', async ({ page, context, request }, testInfo) => {
  ...
  const mapSettled = async () => {
    const where = () =>
      page.evaluate(() => {
        ...
        return p && !document.querySelector('#cl-map.leaflet-zoom-anim')
          ? [Math.round(p.x - m.x), Math.round(p.y - m.y), m.width, m.height]
          : null
      })
    let last = null
    await expect
      .poll(
        ...
        { message: "the map has settled on the client's pin", intervals: [300] },
      )
      .toBe(true)
  }
  ...
  await shot('clients')
  if (!wide) {
    // at phone width the map sits under the list
  ...
  for (const [tabName, ready, name] of [
    ['Billing', '#billing-table', 'report-billing'],
    ['Missed and late', '#missed-table', 'report-missed'],
    ['Mileage', '.mileage-day', 'report-mileage'],
  ]) {
