// npm test: pure unit tests, then the API tests against a TEST_MODE Worker on PORT (default 7902, inspector PORT + 10).
...
export async function startWorker ({ dir = root, port, log }) {
  ...
  // app/public belongs to the app slice and may not exist in this worktree; wrangler refuses a missing assets folder,
...
async function main () {
  ...
  try {
    ...
    status = step(process.execPath, ['--test', '--test-concurrency=1', 'tests/api.test.mjs'], { BASE })
  } finally {
