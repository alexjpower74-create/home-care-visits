// Shared plumbing for the negative controls: copy worker/ to worker/.negative/<name> (git-ignored), apply literal breaks that
// must each match exactly once, run the named tests on the unbroken copy (they must pass) and on the broken copy (they must
// fail), and append a scrubbed record to tests/negative-control.log. The shipped code has no switch for any of these breaks.
...
export const NEGATIVE_PORT = 7905
...
/** A fresh copy of worker/ with the assets directory made absolute so it still resolves from the copy. */
...
export function createLog(title) {
  ...
  say(`\n=== ${title} ${new Date().toISOString()} (HEAD ${sha}${dirty ? ', with uncommitted worker changes' : ''}) ===`)
  return {
    say,
...
/**
 * One control: copy, run the named tests on the unbroken copy (each must pass), apply the breaks, run again (each named test
 * must show ✖; a file that no longer loads is not accepted as red). For API tests the copy is served on 7905.
 * Exits 0 only when the unbroken copy passed and every named test went red on the broken one.
 */
export async function runControl({ title, testFile, tests, breaks, describe, api = false }) {
  ...
  try {
    ...
    const args = ['--test', '--test-concurrency=1', ...nameArgs(tests), testFile]
    let env = {}
    if (api) {
