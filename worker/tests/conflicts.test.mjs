...
const PAT = {
  ...
  name: 'Pat Q. (SAMPLE)',
  zone_ids: [1],
  availability: { 1: WINDOW, 2: WINDOW, 3: WINDOW, 4: WINDOW, 5: WINDOW, 6: null, 7: null },
  max_week_minutes: 200,
}
...
function metresHereToNorth() {
  ...
  const h =
    Math.sin(toRad(NORTH.lat - HERE.lat) / 2) ** 2 +
...
test('conflicts: a hand-built week with exactly one of each kind, exact messages and order', () => {
  ...
  assert.deepEqual(got, [
    ...
    {
      kind: 'travel_gap',
      label: 'Travel gap too short',
      severity: 'problem',
      worker_id: 1,
      date: '2026-09-14',
      visit_ids: [2, 3],
      message:
        'Pat Q. (SAMPLE) has 5 min between Y (SAMPLE) and Z (SAMPLE) on Mon Sep 14, but they are 10.0 km apart in a straight line (about 14 min of driving).',
      gap_minutes: 5,
      needed_minutes: 14,
      distance_m: 10008,
    },
    ...
    {
      kind: 'over_hours',
      label: 'Over weekly hours',
      severity: 'problem',
      worker_id: 1,
      date: null,
      visit_ids: [4, 5],
      message: 'Pat Q. (SAMPLE) is booked for 4.4 h this week; their limit is 3.3 h.',
      scheduled_minutes: 265,
      max_week_minutes: 200,
    },
  ])
})
...
test('conflicts: travel-gap boundary, gap = needed is fine and gap = needed − 1 is a conflict', () => {
  ...
  const pair = (gap) => {
    ...
    return findConflicts({
      visits: [visit({}), visit({ client_name: 'Z (SAMPLE)', ...NORTH, start, end: '11:30' })],
      workers: [worker],
      zones: ZONES,
    })
  }
...
test('conflicts: unassigned and cancelled visits never conflict; the same input in any order gives the same output', () => {
  ...
  for (let seed = 1; seed <= 20; seed++) {
    ...
    assert.deepEqual(
      findConflicts({ visits: shuffled, workers: [{ ...PAT, zone_ids: [...PAT.zone_ids] }], zones: [...ZONES].reverse() }),
      base,
    )
  }
})
...
test('conflicts: over-hours ids run from the visit whose running total passes the limit to the end of the week', () => {
  ...
  assert.deepEqual(
    exactly.map((c) => [c.kind, c.visit_ids]),
    [['over_hours', [4]]],
  )
  ...
  assert.deepEqual(
    early.map((c) => [c.kind, c.visit_ids, c.message]),
    [['over_hours', [2, 3, 4], 'Pat Q. (SAMPLE) is booked for 4 h this week; their limit is 1 h.']],
  )
})
...
export function sampleWeek(weekStart) {
  const workers = SAMPLE.workers.map((w, i) => ({
    id: i + 1,
    name: w.name,
    zone_ids: w.zone_ids,
  ...
  SAMPLE.clients.forEach((c, i) => {
    c.patterns.forEach((p) => {
      patterns.push({
        ...
        worker_id: p.worker,
        valid_from_at: '2020-01-01T00:00:00.000Z',
        ended_at: null,
      })
    })
  })
