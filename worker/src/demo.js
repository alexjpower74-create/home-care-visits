...
const eventId = (visitId, kind) => `00000000-0000-4000-8000-${(visitId * 2 + (kind === 'check_out' ? 1 : 0)).toString(16).padStart(12, '0')}`
...
export async function seedDemo (ctx, ensureVisits) {
  ...
  const history = (visitId, worker) =>
    db.prepare('INSERT OR IGNORE INTO visit_workers (visit_id, worker_id, assigned_at) VALUES (?1, ?2, ?3)').bind(visitId, worker, ctx.nowIso)
  ...
  async function oneOff (clientPrefix, workerPrefix, startMs, minutes) {
    ...
    const row = await db.prepare(`INSERT INTO visits (client_id, pattern_id, pattern_date, date, start_hm, end_hm, starts_at, ends_at, worker_id,
      ...
      .bind(clientId(clientPrefix), date, nlHm(startMs), nlHm(endMs), isoSecond(startMs), isoSecond(endMs), worker, ctx.nowIso).first()
    if (worker) await history(row.id, worker).run()
    return row.id
  }

  const visits = (await db.prepare(`SELECT v.*, c.lat, c.lng FROM visits v JOIN clients c ON c.id = v.client_id
    WHERE v.date BETWEEN ?1 AND ?2 AND v.cancelled = 0 ORDER BY v.starts_at, v.id`).bind(lastMonday, sunday).all()).results
  ...
  const inProgress = visits.find(v => v.date === today && v.worker_id !== null && Date.parse(v.starts_at) <= now && Date.parse(v.starts_at) + 30 * MIN > now)
  let open
  if (inProgress) {
    const start = Date.parse(inProgress.starts_at)
    open = { id: inProgress.id, worker_id: inProgress.worker_id, lat: inProgress.lat, lng: inProgress.lng, at: start + Math.min(inProgress.id % 10, Math.floor((now - start) / MIN)) * MIN }
  } else {
  ...
  const insertEvent = (id, v, workerIdValue, kind, atMs, receivedMs, where) => {
    ...
    return db.prepare(`INSERT INTO events (id, visit_id, worker_id, kind, at, at_adjusted, received_at, source, location, lat, lng, accuracy_m,
      distance_m, correction_reason, voided_at) VALUES (?1, ?2, ?3, ?4, ?5, 0, ?6, 'phone', ?7, ?8, ?9, ?10, ?11, NULL, NULL)`)
      .bind(id, v.id, workerIdValue, kind, isoSecond(atMs), isoSecond(Math.min(receivedMs, now)), kind === 'check_in' ? locationStatus(metres).location : null,
        where?.lat ?? null, where?.lng ?? null, where ? 12 : null, kind === 'check_in' ? metres : null)
  }
  ...
  for (const v of done) {
    ...
    own.forEach((t, pos) => stmts.push(db.prepare('INSERT INTO visit_tasks (visit_id, position, task_id, kind, label, done) VALUES (?1, ?2, ?3, ?4, ?5, ?6)')
      .bind(v.id, pos, t.id, t.kind, TASK_LABELS[t.kind], v.id % 3 === 0 && pos === own.length - 1 ? 0 : 1)))
    if (v.id % 2 === 0) {
      stmts.push(db.prepare('INSERT INTO visit_notes (visit_id, text, shareable, event_id, written_at) VALUES (?1, ?2, ?3, ?4, ?5)')
        .bind(v.id, NOTES[(v.id / 2) % NOTES.length], v.id % 6 === 0 ? 1 : 0, eventId(v.id, 'check_out'), isoSecond(checkOut)))
    }
  }
  stmts.push(insertEvent(eventId(open.id, 'check_in'), open, open.worker_id, 'check_in', open.at, open.at, { lat: open.lat + 0.0002, lng: open.lng }))
