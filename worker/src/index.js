...
async function requireOffice(ctx) {
  ...
  const row = await ctx.db
    .prepare('SELECT token_hash FROM sessions WHERE token_hash = ?1 AND expires_at > ?2')
    .bind(hash, ctx.nowIso)
    .first()
...
async function pinGuard(ctx) {
  const row = await ctx.db
    .prepare('SELECT COUNT(*) AS n FROM signin_attempts WHERE ip = ?1 AND at > ?2')
    .bind(ctx.ip, isoSecond(ctx.nowMs - PIN_WINDOW_MS))
    .first()
...
async function signin(ctx) {
  ...
  await ctx.db
    ...
    .bind(await sha256Hex(token), ctx.nowIso, expiresAt)
    .run()
...
function patternView(p) {
  ...
  return {
    ...
    days_label: daysLabel(days),
    time_label: rangeLabel(p.start_hm, p.end_hm),
  }
}
...
async function loadClients(ctx, { id = null, all = false } = {}) {
  ...
  const [clients, tasks, patterns, contacts] = await db.batch([
    prep(
      db,
      `SELECT c.*, z.name AS zone_name, f.name AS funder_name FROM clients c JOIN zones z ON z.id = c.zone_id
      JOIN funders f ON f.id = c.funder_id WHERE ${where} ORDER BY c.name COLLATE NOCASE, c.id`,
      binds,
    ),
...
function validateClient(raw, { isPost, zones, funders, activeWorkerIds }) {
  ...
  const tasks = input.tasks.map((t) => {
    ...
    if (t.detail !== undefined && t.detail !== null && typeof t.detail !== 'string')
      throw badRequest('tasks', 'Keep each task under 80 characters.')
...
const insertPattern = (db, clientSql, clientBind, p, nowIso) =>
  db
    .prepare(
...
async function createClient(ctx) {
  ...
  const stmts = [
    db
      .prepare(`INSERT INTO clients (name, address, lat, lng, zone_id, entry_notes, funder_id, active, family_key)
      VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)`)
      .bind(c.name, c.address, c.lat, c.lng, c.zone_id, c.entry_notes, c.funder_id, c.active ? 1 : 0, key),
    ...c.tasks.map((t, pos) =>
      db
        .prepare(`INSERT INTO client_tasks (client_id, position, kind, detail) VALUES ((${byKey}), ?2, ?3, ?4)`)
        .bind(key, pos, t.kind, t.detail),
    ),
    ...(c.active ? c.patterns : []).map((p) => insertPattern(db, byKey, key, p, ctx.nowIso)),
    ...c.contacts.map((f, pos) =>
      db
        .prepare(`INSERT INTO family_contacts (client_id, position, name, relationship, phone) VALUES ((${byKey}), ?2, ?3, ?4, ?5)`)
...
const samePattern = (stored, p) =>
  p.id === stored.id &&
  JSON.stringify(p.days) === stored.days &&
  p.start === stored.start_hm &&
  p.end === stored.end_hm &&
...
async function updateClient(ctx, idText) {
  ...
  const stmts = [
    db
      .prepare(`UPDATE clients SET name = ?1, address = ?2, lat = ?3, lng = ?4, zone_id = ?5, entry_notes = ?6, funder_id = ?7, active = ?8
      WHERE id = ?9`)
      .bind(c.name, c.address, c.lat, c.lng, c.zone_id, c.entry_notes, c.funder_id, c.active ? 1 : 0, id),
    db
      .prepare(
        `DELETE FROM client_tasks WHERE client_id = ?1 AND id NOT IN (${keepTaskIds.length ? placeholders(keepTaskIds.length, 2) : 'NULL'})`,
      )
      .bind(id, ...keepTaskIds),
    ...c.tasks.map((t, pos) =>
      t.id !== null && ownTaskIds.has(t.id)
        ? db.prepare('UPDATE client_tasks SET position = ?1, kind = ?2, detail = ?3 WHERE id = ?4').bind(pos, t.kind, t.detail, t.id)
        : db
            .prepare('INSERT INTO client_tasks (client_id, position, kind, detail) VALUES (?1, ?2, ?3, ?4)')
            .bind(id, pos, t.kind, t.detail),
    ),
    db.prepare('DELETE FROM family_contacts WHERE client_id = ?1').bind(id),
    ...c.contacts.map((f, pos) =>
      db
        .prepare('INSERT INTO family_contacts (client_id, position, name, relationship, phone) VALUES (?1, ?2, ?3, ?4, ?5)')
  ...
  if (ending.length) {
    ...
    stmts.push(
      db
        .prepare(`UPDATE visits SET removed_at = ?1, removed_reason = ?${ending.length + 2} WHERE id IN (${doomed}) RETURNING id`)
...
async function loadWorkers(ctx, { id = null, all = false } = {}) {
  ...
  const [workers, zones] = await db.batch([
    ...
    prep(
      db,
      `SELECT wz.worker_id, z.id, z.name FROM worker_zones wz JOIN zones z ON z.id = wz.zone_id
      WHERE wz.worker_id IN (SELECT w.id FROM workers w WHERE ${where}) ORDER BY z.id`,
      binds,
    ),
  ])
...
async function createWorker(ctx) {
  ...
  await db.batch([
    db
      .prepare('INSERT INTO workers (name, phone, availability, max_week_minutes, active, worker_key) VALUES (?1, ?2, ?3, ?4, ?5, ?6)')
...
async function updateWorker(ctx, idText) {
  ...
  const stmts = [
    db
      .prepare('UPDATE workers SET name = ?1, phone = ?2, availability = ?3, max_week_minutes = ?4, active = ?5 WHERE id = ?6')
  ...
  if (!w.active) {
    ...
    stmts.push(
      ...
      db
        .prepare(
          `INSERT OR IGNORE INTO visit_workers (visit_id, worker_id, assigned_at) SELECT id, worker_id, ?2 FROM visits WHERE ${upcoming}`,
        )
        .bind(id, ctx.nowIso),
...
async function ensureVisits(ctx, from, to) {
  ...
  const patterns = (
    await db
      .prepare(`SELECT p.id, p.client_id, p.days, p.start_hm AS start, p.end_hm AS "end", p.worker_id, p.valid_from_at,
      p.ended_at FROM patterns p JOIN clients c ON c.id = p.client_id WHERE c.active = 1 AND (p.ended_at IS NULL OR p.ended_at > ?1)`)
      .bind(`${addDays(firstMonday, -1)}T00:00:00.000Z`)
      .all()
  ).results
  const rows = []
...
async function loadVisitRecords(db, whereOwn, binds, { all = false } = {}) {
  ...
  const [visits, events, tasks, notes] = await db.batch([
    prep(
      db,
      `SELECT v.*, c.name AS client_name, c.zone_id, c.lat, c.lng, c.address, c.entry_notes, w.name AS worker_name,
      w.phone AS worker_phone FROM visits v JOIN clients c ON c.id = v.client_id LEFT JOIN workers w ON w.id = v.worker_id
      WHERE ${where} ORDER BY v.starts_at, v.id`,
      binds,
    ),
    prep(
      db,
      `SELECT e.*, w.name AS worker_name FROM events e JOIN workers w ON w.id = e.worker_id
      WHERE e.voided_at IS NULL AND e.visit_id IN (${ids})`,
      binds,
    ),
...
async function clientTasksFor(db, clientIds) {
  ...
  const rows = (
    await db
      .prepare(`SELECT * FROM client_tasks WHERE client_id IN (${placeholders(unique.length)}) ORDER BY position, id`)
      .bind(...unique)
      .all()
  ).results
...
async function getDay(ctx) {
  ...
  return json(200, {
    ...
    visits: records.map((r) => officeVisitView(r, ctx.nowMs)),
  })
}
...
async function createVisit(ctx) {
  ...
  const row = await db
    ...
    .bind(client.id, body.date, body.start, body.end, t.starts_at, t.ends_at, workerId, ctx.nowIso)
    .first()
  if (workerId !== null) {
    await db
      .prepare('INSERT OR IGNORE INTO visit_workers (visit_id, worker_id, assigned_at) VALUES (?1, ?2, ?3)')
      .bind(row.id, workerId, ctx.nowIso)
      .run()
  }
...
async function updateVisit(ctx, idText) {
  ...
  if (!isDate(body.date)) throw badRequest('date', 'Pick a date.')
  const t = visitTimes(body.date, body.start, body.end)
  const history = [rec.row.worker_id, workerId]
    .filter((w) => w !== null)
    .map((w) =>
      db
        .prepare(
  ...
  return versionedUpdate(
    ...
    [workerId, body.date, body.start, body.end, t.starts_at, t.ends_at],
    history,
  )
}
...
async function setNoteShareable(ctx, idText) {
  ...
  await ctx.db
    .prepare('UPDATE visit_notes SET shareable = ?1 WHERE visit_id = ?2')
    .bind(body.shareable ? 1 : 0, rec.row.id)
    .run()
  return json(200, officeVisitView(await officeVisit(ctx, rec.row.id), ctx.nowMs))
}
...
async function checkInMileage(db, workerId, date) {
  ...
  const rows = (
    await db
      .prepare(`SELECT e.at, v.id AS visit_id, c.lat, c.lng FROM events e JOIN visits v ON v.id = e.visit_id
...
async function workerVisits(ctx) {
  ...
  const [records, agency, metres, open] = await Promise.all([
    ...
    loadVisitRecords(
      db,
      "v.date = ?1 AND (v.worker_id = ?2 OR EXISTS (SELECT 1 FROM events ce WHERE ce.visit_id = v.id AND ce.kind = 'check_in' AND ce.voided_at IS NULL AND ce.worker_id = ?2))",
      [date, worker.id],
    ),
    loadAgency(db),
    ...
    db
      .prepare(`SELECT DISTINCT v.date FROM visits v
      ...
      ORDER BY v.date`)
      .bind(worker.id, addDays(today, -7), today)
      .all(),
  ])
  const tasks = await clientTasksFor(
    db,
    records.map((r) => r.row.client_id),
  )
  return json(200, {
...
function checkedTasks(tasks) {
  ...
  return tasks.map((t) => {
    if (
      !t ||
      typeof t !== 'object' ||
      !Object.hasOwn(TASK_LABELS, t.kind) ||
...
async function postEvent(ctx) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (stored) {
      return json(200, {
        duplicate: true,
        event: workerEventView(stored),
        visit: await workerVisitById(db, stored.visit_id),
        ...storedRefusals(stored),
      })
    }

    const input = validateEvent(body)
    const visit = await db
      .prepare(`SELECT v.*, c.lat AS client_lat, c.lng AS client_lng,
    ...
    if (input.kind === 'check_in') {
      if (checkIn) {
        throw new HttpError(409, 'already_checked_in', `This visit already has a check-in at ${timeLabel(checkIn.at)}.`, {
          event: workerEventView(checkIn),
        })
      }
      const metres = input.location ? distanceM(input.location.lat, input.location.lng, visit.client_lat, visit.client_lng) : null
      Object.assign(event, {
        location: locationStatus(metres).location,
        lat: input.location?.lat ?? null,
        lng: input.location?.lng ?? null,
        accuracy_m: input.location?.accuracy_m ?? null,
        distance_m: metres,
      })
    } else {
      ...
      if (checkOut) {
        throw new HttpError(409, 'already_checked_out', `This visit already has a check-out at ${timeLabel(checkOut.at)}.`, {
          event: workerEventView(checkOut),
        })
      }
    ...
    if (input.kind === 'check_out') {
      ...
      if (input.note) {
        stmts.push(
          db
            .prepare('INSERT INTO visit_notes (visit_id, text, shareable, event_id, written_at) VALUES (?1, ?2, 0, ?3, ?4)')
...
async function getFamily(ctx, key) {
  ...
  const recent = await db
    .prepare('SELECT COUNT(*) AS n FROM family_lookups WHERE ip = ?1 AND at > ?2')
    .bind(ctx.ip, isoSecond(ctx.nowMs - FAMILY_WINDOW_MS))
    .first()
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      date,
      date_label: dayLabel(date),
      visits: records.filter((r) => r.row.date === date).map((r) => familyVisitView(r, today, ctx.nowMs)),
    }
  })
  return json(200, {
...
async function fixTimes(ctx, idText) {
  ...
  if (resultIn !== null && resultOut !== null && resultOut <= resultIn)
    throw badRequest('check_out_at', 'Check-out has to be after check-in.')
...
async function reportRows(db, from, to) {
  return (
    await db
      .prepare(`SELECT v.id AS visit_id, v.date, v.start_hm AS start, v.end_hm AS "end", v.starts_at, v.cancelled, v.removed_at,
    ...
    WHERE ((ci.at >= ?1 AND ci.at < ?2) OR v.date BETWEEN ?3 AND ?4) AND (v.removed_at IS NULL OR ci.id IS NOT NULL OR co.id IS NOT NULL)`)
      .bind(`${addDays(from, -1)}T00:00:00.000Z`, `${addDays(to, 2)}T00:00:00.000Z`, from, to)
      .all()
  ).results.map((row) => ({
    ...row,
    cancelled: row.removed_at ? 1 : row.cancelled,
    scheduled_minutes: minutesOf(row.end) - minutesOf(row.start),
  }))
}

const REPORTS = {
  payroll: { build: (rows, _ctx, p) => payrollReport(rows, p.from, p.to), csv: payrollCsv },
  billing: { build: (rows, _ctx, p) => billingReport(rows, p.from, p.to), csv: billingCsv },
  missed: { build: (rows, ctx, p) => missedReport(rows, ctx.nowMs, p.from, p.to), csv: missedCsv },
  mileage: { build: (rows, _ctx, p) => mileageReport(rows, p.from, p.to), csv: mileageCsv },
}
