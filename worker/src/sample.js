...
export const SAMPLE_PIN_HASH = {
  hash: 'HL0CykjsPuFaSAXK69aAa30EgjVIx27twS5K1yQtMrg=',
  salt: 'iyOxbAJBo7Emc5z1pg1gxg==',
  iterations: 100000,
}
...
export async function seedSample(db, origin) {
  ...
  const first = await db.batch([
    db
      .prepare(`INSERT INTO agency (id, name, office_phone, timezone, office_label, office_lat, office_lng, pin_hash, pin_salt, pin_iterations)
      ...
      .bind(
        ...
        agency.office.lat,
        agency.office.lng,
        SAMPLE_PIN_HASH.hash,
        SAMPLE_PIN_HASH.salt,
        SAMPLE_PIN_HASH.iterations,
      ),
  ...
  SAMPLE.workers.forEach((w, i) => {
    for (const z of w.zone_ids)
      children.push(db.prepare('INSERT INTO worker_zones (worker_id, zone_id) VALUES (?1, ?2)').bind(workerIds[i], z))
  })
  SAMPLE.clients.forEach((c, i) => {
    c.tasks.forEach((t, pos) => {
      children.push(
        db
          .prepare('INSERT INTO client_tasks (client_id, position, kind, detail) VALUES (?1, ?2, ?3, ?4)')
          .bind(clientIds[i], pos, t.kind, t.detail || ''),
      )
    })
    ...
    c.family_contacts.forEach((f, pos) => {
      children.push(
        db
  ...
  return {
    pin: SAMPLE_PIN,
    workers: SAMPLE.workers.map((w, i) => ({
      id: workerIds[i],
      name: w.name,
      key: workerKeys[i],
      worker_url: workerUrl(origin, workerKeys[i]),
    })),
    clients: SAMPLE.clients.map((c, i) => ({
      id: clientIds[i],
      name: c.name,
      family_key: familyKeys[i],
      family_url: familyUrl(origin, familyKeys[i]),
    })),
  }
}
