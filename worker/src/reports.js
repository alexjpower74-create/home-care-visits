...
export const MILEAGE_NOTE =
  "Straight-line distance between clients, in the order the worker checked in. Not road distance. The drive to the first client and home from the last isn't counted."
...
export function payrollReport(rows, from, to) {
  ...
  return {
    ...
    incomplete: rows
      ...
      .map((r) => ({
        visit_id: r.visit_id,
        worker_name: r.check_in_worker_name,
        client_name: r.client_name,
        date_label: dayLabel(r.date),
        check_in_label: timeLabel(r.check_in_at),
      })),
    note: PAYROLL_NOTE,
  }
}
    ...
    funders: groupSorted(done, 'funder_id', 'funder_name').map((f) => ({
      ...
      clients: groupSorted(f.items, 'client_id', 'client_name').map((c) => {
        ...
        return {
          ...
          client_name: c.name,
          visits: t.visits,
          scheduled_minutes: scheduledMinutes(c.items),
          scheduled_hours: scheduledHours(c.items),
          seconds: t.seconds,
          hours: t.hours,
          hm_label: t.hm_label,
        }
...
export function mileageReport(rows, from, to) {
  ...
  for (const r of rows) {
    ...
    if (!days.has(key))
      days.set(key, { worker_id: r.check_in_worker_id, worker_name: r.check_in_worker_name, date: nlDate(r.check_in_at), list: [] })
