...
export function availabilityLabel (availability) {
  ...
  return groups.map(g => `${g.from === g.to ? dayName(g.from) : `${dayName(g.from)}–${dayName(g.to)}`} ${rangeLabel(g.start, g.end)}`).join(', ')
}
...
function conflict (kind, workerId, date, visitIds, message, extra = {}) {
  const k = KIND[kind]
  return { kind, label: k.label, severity: k.severity, worker_id: workerId, date, visit_ids: [...visitIds].sort((a, b) => a - b), message, ...extra }
}
...
export function findConflicts ({ visits, workers, zones }) {
  ...
  for (const [workerId, list] of byWorker) {
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (toMs(a.starts_at) < toMs(b.ends_at) && toMs(b.starts_at) < toMs(a.ends_at)) {
          out.push(conflict('double_booked', workerId, a.date, [a.id, b.id],
    ...
    for (let i = 0; i + 1 < list.length; i++) {
      ...
      if (gap < needed) {
        out.push(conflict('travel_gap', workerId, a.date, [a.id, b.id],
    ...
    if (firstOver >= 0) {
      out.push(conflict('over_hours', workerId, null, list.slice(firstOver).map(v => v.id),
